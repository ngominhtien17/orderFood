...
namespace OrderFood.Helpper
{
    public static class Utilities
    {
        ...
        {
            url = url.ToLower();
            url = Regex.Replace(url, @"[áàààãââââããăăăăăẵ]", "a");
            url = Regex.Replace(url, @"[éèèèèêêêêêễ]", "e");
            url = Regex.Replace(url, @"[óòóõôôôôôÕ¤œàợởõ]", "o");
            url = Regex.Replace(url, @"[íìiiĩ]", "i");
            url = Regex.Replace(url, @"[ýỳyiỹ]", "y");
